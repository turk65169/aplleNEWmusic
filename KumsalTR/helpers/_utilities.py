...
class Utilities:
    ...
    async def play_log(
        ...
        user_id = m.from_user.id if m.from_user else "Kanal"
        user_mention = m.from_user.mention if m.from_user else "Kanal"
        ...
        if chat:
            ...
            return await app.send_message(
                ...
                text=m.lang["log_chat"].format(
                    ...
                    user.mention if user else "Apple Music",
                ),
            )
    ...
    async def safe_edit(self, message: types.Message, text: str, **kwargs) -> types.Message | None | bool:
        try:
