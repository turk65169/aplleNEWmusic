    ...
    if url:
        if "playlist" in url:
            ...
            tracks = await yt.playlist(
                config.PLAYLIST_LIMIT, mention, m.from_user.id, url, video
            )
            ...
            query = await yt.resolve_spotify(url)
            if query:
                file = await yt.search(query, sent.id, user=mention, user_id=m.from_user.id, video=video)
            else:
                file = await yt.search(url, sent.id, user=mention, user_id=m.from_user.id, video=video)
        else:
            file = await yt.search(url, sent.id, user=mention, user_id=m.from_user.id, video=video)

        if not file:
            return await sent.edit_text(
                m.lang["play_not_found"].format(config.SUPPORT_CHAT.split("/")[-1])
            )
